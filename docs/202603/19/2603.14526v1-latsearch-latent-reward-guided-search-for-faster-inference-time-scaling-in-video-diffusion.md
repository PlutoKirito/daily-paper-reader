...
title_zh: LatSearch：用于视频扩散模型中更快推理时扩展的潜空间奖励引导搜索
...
## 摘要
大语言模型中推理时扩展（inference-time scaling）的近期成功，激发了在视频扩散模型中的类似探索。特别是，受能够提升视频质量的“黄金噪声”（golden noise）存在的启发，先前的工作尝试通过优化或搜索更好的初始噪声来改进推理。然而，这些方法存在显著局限性：它们要么依赖于在噪声采样开始时施加的先验，要么依赖于仅在去噪和解码后的视频上评估的奖励。这导致了误差累积、延迟且稀疏的奖励信号，以及极高的计算成本，从而阻碍了更强搜索算法的使用。关键在于，如果能降低计算成本，更强的搜索算法正是解锁视频扩散模型在可控性、采样效率和生成质量方面实质性提升的关键。为了填补这一空白，我们通过潜空间奖励引导（latent reward guidance）实现了视频扩散模型的高效推理时扩展，在去噪轨迹上提供中间的、信息丰富的且高效的反馈。我们引入了一个潜空间奖励模型，该模型可以在任意时间步对部分去噪的潜变量（latents）在视觉质量、运动质量和文本对齐度方面进行评分。基于该模型，我们提出了 LatSearch，这是一种执行奖励引导重采样与剪枝（Reward-Guided Resampling and Pruning, RGRP）的新型推理时搜索机制。在重采样阶段，根据奖励归一化的概率对候选样本进行采样，以减少对奖励模型的过度依赖。在应用于最终预定步骤的剪枝阶段，仅保留累积奖励最高的候选样本，从而同时提高质量和效率。我们在 VBench-2.0 基准测试上评估了 LatSearch，并证明与基准 Wan2.1 模型相比，它在多个评估维度上一致地改进了视频生成。
