...
tags: ["query:vcodec"]
score: 6.0
evidence: 利用扩散 Transformer 联合建模视频动力学
...
## 摘要
视觉-语言-动作（VLA）模型已成为机器人学习中一种极具前景的范式，但其表示在很大程度上仍继承自静态图像-文本预训练，导致物理动力学只能从相对有限的动作数据中学习。相比之下，生成式视频模型编码了丰富的时空结构和隐式物理规律，使其成为机器人操作的有力基础。然而，现有文献尚未充分挖掘其潜力。为了弥补这一差距，我们提出了 DiT4DiT，这是一种端到端的视频-动作模型，它在一个统一的级联框架中将视频扩散 Transformer（Diffusion Transformer）与动作扩散 Transformer 耦合在一起。DiT4DiT 并不依赖于重建的未来帧，而是从视频生成过程中提取中间去噪特征，并将其作为具有时间依据的条件用于动作预测。我们进一步提出了一种具有解耦时间步和噪声尺度的双流匹配（dual flow-matching）目标函数，用于视频预测、隐藏状态提取和动作推理，从而实现两个模块的连贯联合训练。在仿真和真实世界基准测试中，DiT4DiT 均取得了最先进的结果，在 LIBERO 上达到了 98.6% 的平均成功率，在 RoboCasa GR1 上达到了 50.8%，且使用的训练数据大幅减少。在宇树（Unitree）G1 机器人上，它也表现出了卓越的真实世界性能和强大的零样本泛化能力。重要的是，DiT4DiT 将样本效率提高了 10 倍以上，并将收敛速度加快了多达 7 倍，证明了视频生成可以作为机器人策略学习的有效扩展代理（scaling proxy）。我们在 https://dit4dit.github.io/ 发布了代码和模型。
