...
method: 提出实时检查模块将潜变量快速转为预览视频进行评估，并结合分层早期退出机制对预测失败的生成进行干预。
result: 在多个主流模型上实验证明，该方法在提升生成一致性的同时，比事后重生成节省了高达2.64倍的时间开销。
conclusion: 故障信号在去噪早期即可通过中间预览视频检测到，该即插即用流水线为高效视频生成提供了有效方案。
---

## 摘要
文本生成视频（T2V）扩散模型已取得飞速进展，但在实际应用中，生成的视频仍偶尔会出现失败，例如文本-视频对齐度低或感知质量差。由于扩散采样具有非确定性，在推理过程中很难预知生成是否会成功，导致因反复试错重新生成而产生高昂的计算成本。为解决这一问题，我们为潜空间 T2V 扩散模型提出了一种早期故障检测与诊断干预流水线。在检测方面，我们设计了一个实时检查（RI）模块，将潜变量转换为中间视频预览，从而能够在 RGB 空间中使用成熟的文本-视频对齐评分器进行检查。RI 模块仅需 39.2 毫秒即可完成转换和检查过程。考虑到在 NVIDIA A100 GPU 上生成 480p、49 帧视频时，CogVideoX-5B 每步去噪需要 4.3 秒，这一效率极高。随后，我们仅在预测到失败时才触发分层且具有早期退出机制的干预流水线。在 CogVideoX-5B 和 Wan2.1-1.3B 上的实验表明，与事后重新生成相比，该方法在 VBench 上实现了显著的一致性提升，且时间开销减少了多达 2.64 倍。我们的方法还可以推广到更高容量的设置，在 720p 分辨率和 81 帧生成的 Wan2.1-14B 上依然有效。此外，我们的流水线是即插即用的，且与现有技术正交，展示了与提示词优化和采样指导方法的无缝兼容性。我们还提供证据表明，故障信号在去噪过程的早期就会出现，并且可以使用标准的视觉语言评估器在中间视频预览中检测到。
