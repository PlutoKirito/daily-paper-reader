...
tags: ["query:vcodec"]
score: 7.0
evidence: 端到端立体视频生成，联合学习外观与几何结构
...
## 摘要
我们提出了 StereoWorld，这是一个以相机为条件的立体世界模型，它共同学习外观和双目几何，用于端到端的立体视频生成。与单目 RGB 或 RGBD 方法不同，StereoWorld 仅在 RGB 模态下运行，同时直接从视差（disparity）中建立几何基础。为了高效地实现一致的立体生成，我们的方法引入了两个关键设计：(1) 统一的相机帧 RoPE，通过相机感知的旋转位置编码增强潜在标记（latent tokens），在通过稳定的注意力初始化保留预训练视频先验的同时，实现相对的、视图和时间一致的条件约束；(2) 立体感知注意力分解，将完整的 4D 注意力分解为 3D 视图内注意力加上水平行注意力，利用极线先验（epipolar prior）以大幅降低的计算量捕捉视差对齐的对应关系。在各项基准测试中，相比于强大的“先单目后转换”流水线，StereoWorld 提高了立体一致性、视差准确性和相机运动保真度，实现了 3 倍以上的生成速度提升，并在视角一致性上额外获得了 5% 的增益。除基准测试外，StereoWorld 能够实现无需深度估计或修复（inpainting）的端到端双目 VR 渲染，通过米级深度基础增强具身策略学习，并兼容长视频蒸馏，以实现扩展的交互式立体合成。
