...
tldr: OrbitNVS将新视角合成（NVS）重新定义为轨道视频生成任务，利用预训练视频扩散模型的丰富先验来解决单视角下未观测区域的合成难题。通过引入相机适配器、法向图生成分支以及像素级监督，该方法在保持几何一致性和外观清晰度方面表现出色，在GSO和OmniObject3D基准测试中显著超越了现有技术。
selection_source: fresh_fetch
motivation: 现有NVS方法在处理单视角输入时，难以在保持几何与外观一致性的同时合成高质量的未观测区域。
method: 将NVS转化为轨道视频生成，通过相机适配器实现精确控制，并利用法向图特征引导和像素空间监督来增强几何一致性与图像清晰度。
result: 在GSO和OmniObject3D数据集上，OrbitNVS在单视角设置下的PSNR分别提升了2.9 dB和2.4 dB，显著优于先前方法。
conclusion: OrbitNVS证明了利用视频扩散先验并结合几何引导，是实现高质量、一致性强的新视角合成的有效途径。
---
