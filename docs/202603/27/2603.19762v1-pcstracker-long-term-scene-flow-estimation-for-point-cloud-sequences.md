...
tldr: PCSTracker是首个专为点云序列设计的端到端长程场景流估计框架。针对现有方法在长序列中面临的几何演变、遮挡和误差累积等挑战，该框架引入了迭代几何运动联合优化模块（IGMO）和时空轨迹更新模块（STTU），并采用重叠滑动窗口推理策略。实验表明，PCSTracker在PointOdyssey3D等数据集上取得了领先的精度，并能以32.5 FPS的实时速度运行，显著提升了长程3D运动分析的连贯性。
selection_source: fresh_fetch
motivation: 现有的点云场景流估计方法多局限于两帧处理，难以在长序列中应对几何动态变化、遮挡以及随时间增加的误差累积问题。
method: 提出一种结合迭代几何运动优化、时空轨迹更新及重叠滑动窗口推理的端到端框架，旨在显式建模时间演化并利用广泛的时空上下文。
result: 在合成与真实世界数据集上均达到最优的长程场景流估计精度，且处理速度高达32.5 FPS，优于现有的RGB-D方法。
conclusion: PCSTracker通过创新的时空建模和推理策略，有效解决了长程点云运动估计中的一致性难题，为精细化3D运动分析提供了强有力的工具。
---

## 摘要
点云场景流估计是长期和细粒度3D运动分析的基础。然而，现有方法通常局限于成对设置，在几何演变、遮挡出现和误差累积的情况下，难以在长序列中保持时间一致性。在这项工作中，我们提出了 PCSTracker，这是第一个专门为点云序列中一致的场景流估计而设计的端到端框架。具体而言，我们引入了一个迭代几何运动联合优化模块（IGMO），该模块显式地建模点特征的时间演变，以缓解由动态几何变化引起的对应关系不一致。此外，我们提出了一种时空点轨迹更新模块（STTU），利用广泛的时间上下文来推断遮挡点的合理位置，从而确保连贯的运动估计。为了进一步处理长序列，我们采用了重叠滑动窗口推理策略，交替进行跨窗口传播和窗口内细化，有效地抑制了误差累积并保持了稳定的长期运动一致性。在合成的 PointOdyssey3D 和真实世界的 ADT3D 数据集上进行的大量实验表明，PCSTracker 在长期场景流估计中达到了最佳精度，并保持了 32.5 FPS 的实时性能，同时与基于 RGB-D 的方法相比，展示了卓越的 3D 运动理解能力。
