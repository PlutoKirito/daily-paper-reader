...
tldr: 本研究针对V-JEPA等联合嵌入预测架构在潜空间学习中存在的可解释性鸿沟，提出了AI母语（AIM）框架。AIM作为一种轻量级的被动量化探测工具，能在不修改冻结编码器的情况下将连续潜向量转换为离散符号序列。通过在Kinetics-mini数据集上的物理维度实验，研究发现V-JEPA潜空间具有高度紧凑性，语义差异以分布变化而非硬性边界形式存在。该工作证明了从冻结的JEPA潜空间中提取结构化符号流形的可行性，为构建符号化世界模型奠定了基础。
selection_source: fresh_fetch
motivation: 旨在解决JEPA模型因跳过像素重建而导致其内部学习到的物理结构难以被直观解释和验证的问题。
method: 提出AIM框架，利用无词汇表的被动量化探测技术，在保持编码器冻结的状态下将潜向量转化为离散符号。
result: 实验显示AIM符号分布在抓取角度、几何形状和运动结构上均有显著差异，揭示了潜空间中共享核心表示与渐进式语义编码的特性。
conclusion: 证实了结构化符号流形是冻结JEPA潜空间的固有属性，完成了迈向动作条件符号世界模型四阶段路线图的第一步。
---

## 摘要
使用联合嵌入预测架构 (JEPA) 训练的视频世界模型通过预测潜空间中的掩码区域而非重建像素，获得了丰富的时空表示。这消除了生成模型的视觉验证路径，从而产生了一个结构性的可解释性鸿沟：编码器学习到了无法以任何可检查形式获取的物理结构。现有的探测方法要么在没有结构化中间层的连续空间中运行，要么附加了生成组件，其参数会干扰将行为归因于编码器的过程。我们提出了 AI 母语 (AIM) 框架作为一种被动量化探测器：这是一种轻量级、无词汇表的探测器，它在没有特定任务监督或修改编码器的情况下，将 V-JEPA 2 的连续潜向量转换为离散符号序列。由于编码器保持完全冻结，AIM 码本中的任何符号结构都完全归因于 V-JEPA 2 预训练表示，而非探测器本身。我们通过在 Kinetics-mini 数据集上进行的类别对比实验，从三个物理维度进行了评估：抓取角度、物体几何形状和动作时间结构。AIM 符号分布在所有三个实验中均表现出显著差异（卡方检验 p < 10^{-4}；互信息 MI 为 0.036--0.117 bits，归一化互信息 NMI 为 3-bit 最大值的 1.2--3.9%；JSD 高达 0.342；码本激活率为 62.5%）。实验表明，V-JEPA 2 的潜空间显著紧凑：不同的动作类别共享一个共同的表示核心，语义差异被编码为渐进的分布变化，而非明确的类别边界。这些结果确立了通往动作条件符号世界模型四阶段路线图的第一阶段，证明了结构化符号流形是冻结的 JEPA 潜空间中可发现的属性。
