...
title_zh: 通过动态补丁采样和区块跳过实现显存高效的扩散 Transformer 微调
...
## 摘要
扩散 Transformer (DiTs) 显著提升了文本生成图像 (T2I) 的质量，实现了高质量的个性化内容创作。然而，微调这些模型需要极高的计算复杂度和显存，限制了其在资源受限环境下的实际部署。为了应对这些挑战，我们提出了一种名为 DiT-BlockSkip 的显存高效微调框架，该框架集成了时间步感知的动态补丁采样，并通过预计算残差特征实现了区块跳过。我们的动态补丁采样策略根据扩散时间步调整补丁大小，然后将裁剪后的补丁调整为固定的低分辨率。这种方法减少了前向和后向传播的显存占用，同时允许模型在较高时间步捕捉全局结构，在较低时间步捕捉细粒度细节。区块跳过机制有选择地微调关键的 Transformer 区块，并为跳过的区块预计算残差特征，从而显著降低了训练显存。为了识别用于个性化的关键区块，我们引入了一种基于交叉注意力掩码的区块选择策略。评估结果表明，我们的方法在定性和定量上均实现了具有竞争力的个性化性能，同时大幅减少了显存占用，推动了大规模扩散 Transformer 在端侧设备（如智能手机、物联网设备）上的可行性。
