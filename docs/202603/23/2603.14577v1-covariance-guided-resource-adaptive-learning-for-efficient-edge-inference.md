...
tldr: 针对边缘设备深度学习推理中硬件配置优化难的问题，本文提出CORAL，一种无需离线分析的在线优化方法。CORAL利用距离协方差捕捉硬件设置与性能指标间的非线性依赖，将推理优化建模为吞吐量与功耗的协同优化问题。在NVIDIA Jetson设备上的实验表明，CORAL能以极低的探索成本找到接近最优的配置，有效满足功耗预算和吞吐量目标。
selection_source: fresh_fetch
motivation: 边缘设备推理中相同吞吐量的不同硬件配置功耗差异巨大，且现有方法依赖昂贵的离线分析或低效的静态预设。
method: 提出CORAL方法，利用距离协方差统计建模硬件参数与性能间的非线性关系，实现吞吐量与功耗的在线协同优化。
result: "在多种目标检测模型上，CORAL达到了穷举搜索最优性能的96%至100%，并在双重约束场景下表现显著优于基准方法。"
conclusion: CORAL通过在线学习机制，在无需预先分析的情况下，为边缘推理提供了高效且资源自适应的硬件配置方案。
---
