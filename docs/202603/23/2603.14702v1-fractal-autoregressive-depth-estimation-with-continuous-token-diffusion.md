...
tldr: 本研究针对单目深度估计中自回归生成的效率与稳定性问题，提出了一种分形视觉自回归扩散框架。该框架将深度估计重构为由粗到细的跨尺度自回归生成过程，利用VCFR模块融合多尺度特征，并结合条件去噪扩散损失在连续空间建模以避免量化误差。通过分形递归架构提升计算效率，并引入不确定性感知聚合方案增强推理稳定性。实验证明该方法在标准基准上表现优异。
selection_source: fresh_fetch
motivation: 传统的自回归深度估计面临RGB与深度模态差异、像素级生成效率低下以及连续深度预测不稳定的挑战。
method: 提出分形视觉自回归扩散框架，通过递归重用基础单元实现由粗到细的生成，并结合连续扩散模型与不确定性感知聚合。
result: 在标准基准测试中展现出强大的性能，验证了分形架构和连续空间建模的有效性。
conclusion: 该研究通过结合分形递归与扩散模型，为高效、高精度的连续深度估计提供了一种新颖且稳健的自回归生成方案。
---
