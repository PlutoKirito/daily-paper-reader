...
score: 6.0
evidence: 针对3D高斯泼溅的训练后剪枝方法，旨在降低复杂度
...
figures_json: "[{\"url\": \"assets/figures/arxiv/2603.21933v1/fig-001.webp\", \"caption\": \"\", \"page\": 2, \"index\": 1, \"width\": 1396, \"height\": 706}, {\"url\": \"assets/figures/arxiv/2603.21933v1/fig-002.webp\", \"caption\": \"\", \"page\": 5, \"index\": 2, \"width\": 1249, \"height\": 574}, {\"url\": \"assets/figures/arxiv/2603.21933v1/fig-003.webp\", \"caption\": \"\", \"page\": 11, \"index\": 3, \"width\": 1453, \"height\": 856}]"
...
## 摘要
3D 高斯泼溅（3D Gaussian splats）的剪枝对于降低其复杂性以实现高效存储、传输和下游处理至关重要。然而，大多数现有的剪枝策略都依赖于相机参数、渲染图像或与视角相关的度量。在新兴的相机无关交换场景中，这种依赖性成为了一种障碍，因为在这些场景中，泼溅（splats）直接作为基于点的表示（例如 .ply）进行共享。在本文中，我们提出了一种针对 3D 高斯泼溅的相机无关、单次（one-shot）、训练后剪枝方法，该方法仅依赖于从属性派生的邻域描述符。作为我们的主要贡献，我们引入了一个混合描述符框架，该框架直接从泼溅表示中捕获结构和外观的一致性。基于这些描述符，我们将剪枝建模为一个统计证据估计问题，并引入了一个 Beta 证据模型，通过概率置信度分数来量化每个泼溅的可靠性。在 ISO/IEC MPEG 通用测试条件（CTC）定义的标准化测试序列上进行的实验表明，我们的方法在保持重建质量的同时实现了大幅剪枝，为现有的相机相关剪枝策略提供了一种实用且具有泛化能力的替代方案。
