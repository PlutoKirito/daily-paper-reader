...
motivation: 紧凑型ViT在边缘密集预测任务中因缺乏任务特定表征学习，导致其性能和效率权衡难以与成熟的CNN架构（如YOLO）竞争。
method: 提出EdgeCrafter框架，采用任务专用蒸馏技术优化紧凑型骨干网络，并结合边缘感知的编解码器设计构建高效的密集预测模型。
result: ECDet-S在参数量小于10M的情况下达到51.7 AP，而ECPose-X在姿态估计任务中以74.8 AP显著超越了经过大规模预训练的YOLO系列模型。
conclusion: 通过任务专用蒸馏和边缘优化设计，紧凑型ViT能够成为边缘密集预测任务中极具竞争力的实用方案。
---
