...
## 摘要
第六代（6G）无线网络的兴起有望提供超可靠、低延迟且高能效的通信、感知和计算。然而，传统的集中式人工智能（AI）范式并不适用于 6G 生态系统去中心化、资源受限且动态变化的特性。本文探讨了知识蒸馏（KD）和协作学习作为极具前景的技术，如何在分布式通信与感知（C&S）节点上实现轻量级 AI 模型的高效且可扩展部署。我们首先概述了 KD，并强调了其在以设备异构性、任务多样性和资源受限为特征的分布式场景中表现尤为出色的关键优势。随后，我们研究了其通过各种知识蒸馏和部署策略，在中心节点与分布式节点之间的协作学习中促进集体智能的作用。最后，我们通过一项系统的数值研究表明，赋能 KD 的协作学习可以有效支持多模态感知辅助波束跟踪应用中的轻量级 AI 模型，并显著提升性能、降低复杂度。
