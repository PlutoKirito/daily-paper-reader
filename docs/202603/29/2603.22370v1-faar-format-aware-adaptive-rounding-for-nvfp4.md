...
## 摘要
在边缘设备上部署大语言模型（LLM）需要极低比特量化。诸如 NVFP4 之类的超低精度格式为减少内存占用和加速计算提供了一种极具前景的解决方案。然而，现有的量化方法通常依赖于传统的舍入策略，未能考虑到 NVFP4 数值网格的非均匀性，导致舍入决策并非最优且放大了量化误差。为此，我们提出了格式感知自适应舍入（FAAR），这是一种专为 NVFP4 格式定制的可学习舍入策略。与传统的量化范式不同，FAAR 将非均匀的 NVFP4 网格显式地纳入优化过程。通过在损失梯度的引导下自适应地调整舍入决策，我们的方法有效地逼近了理论上的最优量化。为了补充 FAAR，我们引入了一种两阶段格式对齐（2FA）微调方案，该方案将 LLM 参数逐层对齐到 NVFP4 数值空间，从而进一步缩小性能差距。值得注意的是，这种可学习的优化在 Llama3-1B 上仅产生 4 个 GPU 小时的极小训练开销。广泛的实验证明了我们方法的有效性。与最近邻舍入（RTN）相比，我们的方法在 WikiText-2 数据集上将 Llama3-1B 的困惑度从 14.28 降低到 12.60，将 Qwen3-1.7B 的困惑度从 23.06 降低到 21.27。此外，我们的方法在各种零样本下游任务中始终优于最先进的方法。
