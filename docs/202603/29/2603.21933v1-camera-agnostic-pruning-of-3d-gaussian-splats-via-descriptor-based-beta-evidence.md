...
tldr: 针对3D高斯泼溅（3DGS）剪枝过度依赖相机参数的问题，本文提出一种相机无关的单次后训练剪枝方法。该方法通过构建捕获结构和外观一致性的混合描述符，并引入Beta证据模型来量化每个高斯点的可靠性概率得分，实现了在不依赖渲染视图的情况下高效压缩3DGS，同时保持了较高的重建质量，为3DGS的存储和传输提供了新方案。
selection_source: fresh_fetch
motivation: 现有的3DGS剪枝策略大多依赖相机参数或渲染图像，限制了其在直接以点云格式共享等相机无关场景下的应用。
method: 提出一种基于属性派生邻域描述符的混合框架，并利用Beta证据模型通过概率置信度评分来评估每个高斯点的可靠性。
result: 在ISO/IEC MPEG通用测试条件下的实验证明，该方法在实现大幅度剪枝的同时，能够有效保留场景的重建质量。
