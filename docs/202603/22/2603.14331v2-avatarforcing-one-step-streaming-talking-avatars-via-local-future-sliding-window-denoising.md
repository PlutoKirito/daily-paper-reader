...
title_zh: AvatarForcing：通过局部未来滑动窗口去噪实现的一步流式对话头像生成
authors: "Liyuan Cui, Wentao Hu, Wenyuan Zhang, Zesong Yang, Fan Shi, Xiaoqiang Liu"
date: 2026-03-15
pdf: "https://arxiv.org/pdf/2603.14331v2"
tags: ["query:vcodec"]
score: 7.0
evidence: 用于视频生成的单步流式扩散框架
tldr: 针对实时对话头像生成中自回归模型易产生误差累积和全序列扩散模型计算开销大的问题，本文提出AvatarForcing。这是一个单步流式扩散框架，通过局部未来滑动窗口去噪和双锚点时间强制技术，实现了低延迟且长时稳定的视频生成。结合两阶段流式蒸馏，该方法在保持高视觉质量和口型同步的同时，实现了每帧34毫秒的实时推理速度，有效解决了长序列生成的漂移问题。
selection_source: fresh_fetch
motivation: 旨在解决自回归模型在长序列生成中的曝光偏差导致的误差累积问题，以及扩散模型在实时流式推理中的高计算延迟。
method: 提出一种单步流式扩散框架，利用局部未来滑动窗口去噪、双锚点时间强制技术以及两阶段流式蒸馏来实现高效稳定的生成。
result: 在标准及长视频基准测试中，1.3B参数的模型达到了每帧34毫秒的推理速度，并展现出优异的视觉质量和口型同步性能。
conclusion: AvatarForcing成功平衡了生成质量、长时稳定性和实时性，为大规模实时流式对话头像合成提供了有效的解决方案。
---

## 摘要
实时对话头像生成需要低延迟和分钟级的时序稳定性。自回归（AR）强制（forcing）虽然支持流式推理，但存在曝光偏差，导致误差在长序列生成中不断累积且不可逆。相比之下，全序列扩散 Transformer 虽然能缓解漂移，但在实时长视频合成中计算开销过大。我们提出了 AvatarForcing，这是一个一步流式扩散框架，它通过异构噪声水平对固定的局部未来窗口进行去噪，并在恒定的每步成本下每步输出一个清晰块。为了稳定无限流，该方法引入了双锚点时序强制：一个是风格锚点，通过重新索引 RoPE 以保持与当前窗口的固定相对位置，并应用锚点音频零填充；另一个是时序锚点，通过重用最近输出的清晰块来确保平滑过渡。通过带有离线 ODE 回填和分布匹配的两阶段流式蒸馏，实现了实时一步推理。在标准基准测试和包含 400 个视频的新长视频基准测试上的实验表明，使用 1.3B 参数的学生模型进行实时流式传输，在 34 毫秒/帧的速度下展现出强大的视觉质量和口型同步性。我们的项目主页位于：https://cuiliyuan121.github.io/AvatarForcing/
