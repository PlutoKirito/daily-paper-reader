...
tags: ["query:vcodec"]
score: 6.0
evidence: 扩散模型采样调度的图论规划
tldr: 扩散模型通常采用固定的采样策略，导致计算资源分配不均。本文提出 Chain-of-Trajectories (CoTj)，一种无需训练的 System 2 规划框架。通过引入“扩散 DNA”作为低维特征来量化去噪难度，将采样过程建模为有向无环图上的路径规划。该方法采用“预测-规划-执行”范式，动态分配计算资源，在提升生成质量与稳定性的同时，显著减少了冗余计算。
selection_source: fresh_fetch
motivation: 扩散模型现有的固定采样调度忽略了不同生成阶段的难度差异，导致高维空间下的计算资源分配效率低下。
method: 提出 CoTj 框架，利用“扩散 DNA”量化去噪难度，并通过图论规划在有向无环图上动态寻找最优采样轨迹。
result: 实验证明 CoTj 能发现上下文感知的采样路径，在多种生成模型上均提升了输出质量并降低了计算开销。
conclusion: 该研究为基于规划的资源感知型扩散模型奠定了基础，实现了从固定调度到启发式决策的转变。
---

## 摘要
扩散模型以反射式的“系统 1”模式运行，受限于固定的、与内容无关的采样调度。这种僵化源于状态维度的诅咒，高维噪声流形中可能状态的组合爆炸使得显式轨迹规划变得难以处理，并导致系统性的计算分配失当。为了解决这一问题，我们引入了轨迹链（Chain-of-Trajectories, CoTj），这是一个支持“系统 2”审慎规划的免训练框架。CoTj 的核心是 Diffusion DNA，这是一种量化各阶段去噪难度的低维特征，并作为高维状态空间的代理，使我们能够将采样重新表述为有向无环图上的图规划。通过“预测-规划-执行”范式，CoTj 将计算资源动态分配给最具挑战性的生成阶段。在多个生成模型上的实验表明，CoTj 能够发现上下文感知的轨迹，在减少冗余计算的同时提高输出质量和稳定性。这项工作为资源感知、基于规划的扩散建模奠定了新基础。代码可在 https://github.com/UnicomAI/CoTj 获取。
