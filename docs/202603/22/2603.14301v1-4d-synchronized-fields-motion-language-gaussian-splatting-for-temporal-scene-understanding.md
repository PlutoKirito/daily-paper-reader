...
## 摘要
当前的4D表示方法将几何、运动和语义解耦：重建方法丢弃了可解释的运动结构；基于语言的方法在学习运动后才附加语义，无法感知物体的运动方式；而运动感知方法将动力学编码为不透明的逐点残差，缺乏物体层级的组织。我们提出了4D同步场（4D Synchronized Fields），这是一种4D高斯表示，它在重建过程中同步学习物体分解的运动，并通过逐物体条件场将语言与生成的运动学同步。每个高斯轨迹被分解为共享的物体运动加上隐式残差，且运动学条件的脊图（ridge map）预测时序语义变化，从而产生一个单一的表示，其中重建、运动和语义在结构上耦合，并支持检索物体和时刻的开放词汇时序查询。在HyperNeRF数据集上，4D同步场实现了28.52 dB的平均PSNR，在所有基于语言和运动感知的基准方法中排名最高，与仅重建的方法差距在1.5 dB以内。在目标时序状态检索任务中，运动学条件场达到了0.884的平均准确率、0.815的平均vIoU和0.733的平均tIoU，显著超过了4D LangSplat（分别为0.620、0.433和0.439）和LangSplat（0.415、0.304和0.262）。消融实验证实，运动学条件是性能提升的主要驱动因素，相比仅静态嵌入的基准，tIoU提升了0.45。4D同步场是唯一一种能从单一训练表示中同时提供可解释运动原语和时序对齐语言场的方法。代码将会开源。
