...
conclusion: 该研究证明了在标准CPU硬件上，通过合理的量化感知训练技术，4位量化模型可以达到与全精度模型相当的性能水平。
---

## 摘要
低精度神经网络训练已成为降低计算成本和普及深度学习研究的一个极具前景的方向。然而，现有的 4 位量化方法要么依赖昂贵的 GPU 基础设施，要么面临严重的精度下降。在这项工作中，我们提出了一种实用的方法，利用标准 PyTorch 操作在商用 CPU 上以真正的 4 位精度训练卷积神经网络。我们引入了一种新型的基于 tanh 的软权重裁剪技术，结合对称量化、动态逐层缩放和直通估计器（straight-through estimators），实现了稳定的收敛和具有竞争力的精度。在 CIFAR-10 数据集上从头开始训练一个拥有 325 万个参数的 VGG 风格架构，我们的方法在 Google Colab 的免费 CPU 层级上达到了 92.34% 的测试精度——与全精度基准性能（92.5%）相比仅有 0.16% 的差距。我们进一步在 CIFAR-100 上进行了验证，使用相同的架构和训练过程在 100 个类别中实现了 70.94% 的测试精度，证明了从头开始的 4 位训练可以推广到更难的分类任务。两次实验均实现了相对于 FP32 的 8 倍内存压缩，同时在整个训练过程中每层保持精确的 15 个唯一权重值。我们还通过在消费级移动设备（一加 9R）上演示快速收敛来验证硬件独立性，仅用 6 个 epoch 就达到了 83.16% 的精度。据我们所知，此前尚无研究在不使用专用内核或训练后量化的情况下，在标准 CPU 硬件上证明 4 位量化感知训练能够达到全精度性能水平。
