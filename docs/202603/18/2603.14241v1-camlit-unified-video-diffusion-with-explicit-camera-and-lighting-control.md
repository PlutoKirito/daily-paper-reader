...
tags: ["query:vcodec"]
score: 8.0
evidence: 用于新视角合成和时间一致性的统一视频扩散模型
...
## 摘要
我们提出了 CamLit，这是首个能够从单张输入图像中联合执行新视角合成（NVS）和重打光的统一视频扩散模型。给定一张参考图像、用户定义的相机轨迹和环境贴图，CamLit 能够在指定光照下合成场景在新视角下的视频。在单个生成过程中，我们的模型产生时间上相干且空间上对齐的输出，包括重打光后的新视角帧和相应的反照率（albedo）帧，从而实现了对相机姿态和光照的高质量控制。定性和定量实验表明，CamLit 在新视角合成和重打光任务中均实现了与现有最先进方法相当的高保真输出，且未牺牲任何一项任务的视觉质量。我们证明了单个生成模型可以有效地整合相机和光照控制，在简化视频生成流程的同时，保持了具有竞争力的性能和一致的真实感。
