...
tags: ["query:vcodec"]
score: 7.0
evidence: 用于视频生成的扩散蒸馏与时间运动一致性
tldr: 针对扩散模型在实时视频流生成中存在的计算开销大、时序一致性差等问题，本文提出了一种名为“对角蒸馏”的自回归视频生成方法。该方法通过非对称生成策略，在早期视频块使用较多去噪步数以确立基础特征，后期块减少步数以提升效率，并结合隐式光流建模。该方案有效缓解了长序列生成中的误差累积和过饱和问题，在保证运动质量的同时实现了极高的生成速度。
selection_source: fresh_fetch
motivation: 现有的视频蒸馏方法大多简单套用图像技术，忽略了时序依赖，导致长视频生成中出现运动不连贯和严重的误差累积。
method: 提出对角蒸馏策略，通过非对称的去噪步数分配和隐式光流建模，使后续视频块能从前期充分处理的块中继承丰富信息。
result: 该方法实现了比原始模型快277.3倍的生成速度，仅需2.61秒即可生成5秒视频，帧率高达31 FPS。
conclusion: 对角蒸馏通过优化时序上下文利用和去噪步数分配，为高效、高质量的实时流式视频生成提供了创新的解决方案。
---
