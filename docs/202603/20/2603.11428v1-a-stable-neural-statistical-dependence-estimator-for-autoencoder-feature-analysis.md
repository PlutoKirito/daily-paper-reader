...
## 摘要
互信息等统计依赖度量是分析自动编码器的理想工具，但对于确定性、静态、无噪声的网络，这类度量可能是病态的。我们采用了变分（高斯）公式，使得输入、潜变量和重构之间的依赖关系变得可测量，并提出了一种基于正交密度比分解的稳定神经依赖估计器。与 MINE 不同，我们的方法避免了输入拼接和边缘乘积重配对，从而降低了计算成本并提高了稳定性。我们引入了一种高效的类 NMF 标量目标函数，并通过实验证明，假设高斯噪声以形成辅助变量可以实现有意义的依赖性测量，并支持定量特征分析，且伴随着奇异值的顺序收敛。
