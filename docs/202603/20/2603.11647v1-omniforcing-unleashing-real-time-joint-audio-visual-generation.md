...
tldr: 针对现有音视频扩散模型因双向注意力导致的高延迟问题，本文提出OmniForcing框架。该框架首次将离线双流双向扩散模型蒸馏为高保真流式自回归生成器。通过引入非对称块因果对齐、音频Sink Token机制及联合自强力蒸馏范式，解决了多模态信息密度差异导致的训练不稳定和同步漂移问题。OmniForcing在单GPU上实现了约25 FPS的实时流式生成，且音视频同步与质量可媲美离线教师模型。
selection_source: fresh_fetch
motivation: 现有的音视频联合生成模型因双向注意力依赖导致推理延迟高，难以满足实时应用需求。
method: 提出一种结合非对称块因果对齐、音频Sink Token及联合自强力蒸馏的流式自回归蒸馏框架。
result: 在单GPU上实现了约25 FPS的实时生成速度，并保持了与离线模型相当的音视频同步性和视觉质量。
conclusion: OmniForcing成功解决了流式音视频生成的训练不稳定性与累积误差问题，为实时多模态交互提供了高效方案。
---
