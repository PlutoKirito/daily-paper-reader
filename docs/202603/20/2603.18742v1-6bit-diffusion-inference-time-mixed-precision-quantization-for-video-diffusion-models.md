...
score: 8.0
evidence: 视频扩散模型的混合精度量化
tldr: 针对视频扩散模型（Video DiTs）显存占用高和计算开销大的问题，本文提出了一种推理时混合精度量化框架。通过发现模块输入输出差异与量化敏感度之间的线性关系，设计了轻量级预测器动态分配NVFP4和INT8精度。同时利用时间冗余性引入时间增量缓存（TDC）跳过冗余计算，在保证生成质量的同时显著提升了推理效率。
selection_source: fresh_fetch
motivation: 现有的量化方法采用静态位宽分配，忽略了不同扩散步长下激活值的量化难度差异，导致效率与质量的权衡不佳。
method: 提出一种基于输入输出差异预测敏感度的动态混合精度量化策略，并结合时间增量缓存（TDC）技术减少冗余计算。
result: 实验表明该方法实现了1.92倍的端到端加速和3.32倍的显存缩减，且不损失生成质量。
conclusion: 该框架为视频扩散模型的高效推理设定了新基准，证明了动态精度分配和时间冗余利用在模型压缩中的有效性。
---
