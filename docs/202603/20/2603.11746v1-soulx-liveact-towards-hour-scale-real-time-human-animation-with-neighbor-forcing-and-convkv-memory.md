...
title_zh: SoulX-LiveAct：通过邻域强迫与 ConvKV 记忆实现小时级实时人物动画
...
## 摘要
自回归 (AR) 扩散模型通过结合扩散建模与因果推理，为视频合成等序列生成任务提供了一个极具前景的框架。尽管它们支持流式生成，但现有的 AR 扩散方法在高效扩展方面仍面临挑战。在本文中，我们识别了小时级实时人物动画中的两个关键挑战。首先，大多数强迫策略传播具有不匹配扩散状态的样本级表示，导致学习信号不一致且收敛不稳定。其次，历史表示呈无界增长且缺乏结构，阻碍了缓存状态的有效重用，并严重限制了推理效率。为了应对这些挑战，我们提出了 Neighbor Forcing，这是一种扩散步长一致的 AR 公式，它在相同的噪声条件下将时间相邻的帧作为潜邻域进行传播。这种设计提供了一个分布对齐且稳定的学习信号，同时在整个 AR 链中保留了漂移。在此基础上，我们引入了一种结构化的 ConvKV 记忆机制，将因果注意力中的键和值压缩为固定长度的表示，从而实现常数内存推理和真正的无限视频生成，而不依赖于短期运动帧记忆。大量实验表明，与现有的 AR 扩散方法相比，我们的方法显著提高了训练收敛性、小时级生成质量和推理效率。在数值上，LiveAct 实现了小时级实时人物动画，并支持在仅两块 NVIDIA H100 或 H200 GPU 上进行 20 FPS 的实时流式推理。定量结果表明，我们的方法在口型同步准确度、人物动画质量和情感表达力方面达到了最先进的性能，且推理成本最低。
