...
score: 6.0
evidence: 用于鲁棒表示学习的知识蒸馏
tldr: 本研究针对自监督学习在失真数据上表现受限的问题，提出了一种基于预训练视觉Transformer（ViT）的非对称知识蒸馏框架。该框架通过让教师模型处理清晰图像、学生模型处理对应的失真图像，并在全局嵌入、补丁特征及注意力图等多个层级进行对齐，使学生模型能够学习到鲁棒的视觉表示。实验证明，该方法在多种失真场景下的分类任务中均优于现有技术。
source: arxiv
...
motivation: 旨在解决自监督学习在缺乏清晰观测数据时难以学习到鲁棒视觉表示的挑战。
method: 采用非对称知识蒸馏，通过多层级对齐将预训练教师模型的清晰图像知识迁移给处理失真图像的学生模型。
result: 在不同数据集和多种失真条件下的图像分类任务中，该方法一致性地超越了现有的替代方案。
conclusion: 证明了利用预训练模型进行跨失真层级的知识蒸馏是提升模型在恶劣环境下鲁棒性的有效途径。
---

## 摘要
自监督学习在从干净数据中学习视觉表征方面取得了显著成功，但在干净观测数据稀疏或完全不可用时仍然面临挑战。在本文中，我们证明了可以利用预训练视觉模型来学习抗失真表征，这些表征随后可以有效地应用于处理失真观测数据的下游任务。具体而言，我们提出了一种非对称知识蒸馏框架，其中教师和学生模型均初始化自同一个预训练 Vision Transformer，但接收每张图像的不同视图：教师处理干净图像，而学生观察其失真版本。我们引入了多级蒸馏，对齐了全局嵌入、补丁级特征和注意力图，并展示了学生模型尽管从未直接接触干净数据，却能够逼近干净图像的表征。我们在多个数据集和各种失真情况下的图像分类任务上评估了我们的方法，在相同的人工监督量下，其表现始终优于现有的替代方案。
