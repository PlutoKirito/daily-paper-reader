...
tags: ["query:vcodec"]
score: 6.0
evidence: 4D重建中的可扩展运动场与运动一致性
tldr: 针对单目视频中复杂动态场景的4D重建难题，本文提出MotionScale框架。该方法基于4D高斯泼溅，通过可扩展的运动场和分阶段渐进式优化策略，有效解决了长序列中的几何精度与运动一致性问题。其核心贡献在于引入了基于聚类中心的基变换运动参数化方法，以及针对背景扩展和前景传播的解耦优化流程，在保持高保真度的同时实现了大规模场景的稳定重建。
source: arxiv
...
motivation: 现有神经渲染方法在处理复杂环境的长视频时，难以恢复准确的3D几何结构并保持时间上的运动一致性。
method: 提出一种由聚类中心基变换参数化的可扩展运动场，并结合背景扩展与前景传播的两阶段渐进式优化策略。
result: 在多个挑战性的真实世界基准测试中，MotionScale在重建质量和时间稳定性方面均显著优于现有最先进方法。
conclusion: MotionScale为大规模动态场景的高保真4D重建提供了一种高效且具有良好扩展性的解决方案。
---
