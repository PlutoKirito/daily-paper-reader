...
method: 提出将密集点轨迹作为视觉标记，并结合扩散变换器对轨迹集进行建模，以显式处理遮挡并预测运动模式。
result: 该方法在300小时的大规模动物视频数据集上表现优异，实现了类别无关的预测，并能有效泛化至稀有物种。
conclusion: 密集点轨迹作为一种中层结构化表示，为实现自然环境下鲁棒且通用的视觉预测智能提供了有效路径。
---
