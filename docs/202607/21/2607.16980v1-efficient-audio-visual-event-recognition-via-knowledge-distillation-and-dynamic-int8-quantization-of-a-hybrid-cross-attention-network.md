...
title_zh: 基于知识蒸馏与动态INT8量化的混合交叉注意力网络的高效音视频事件识别
...
tldr: "音频-视觉事件识别中Transformer多模态模型计算复杂度过高，限制边缘部署。本文提出结合结构压缩、知识蒸馏和动态INT8量化的压缩框架：教师模型采用VideoMAE、AST和混合交叉注意力融合，学生模型缩减隐藏维度和注意力头数，通过蒸馏学习后量化。在AVE数据集上，多模态融合模块参数量减少59.06%，准确率仅下降2.14%；量化后模型从10.71MB降至2.04MB。该方法有效平衡识别精度与计算效率，适合资源受限的边缘AI平台。"
source: arxiv
...
motivation: 解决Transformer多模态模型在边缘设备部署时的高计算复杂度和内存占用问题。
method: 结合混合交叉注意力网络结构压缩、知识蒸馏和动态INT8量化，构建高效学生模型。
result: "参数量减少59.06%，准确率仅降2.14%；模型大小从10.71MB压缩至2.04MB。"
conclusion: 所提框架在识别精度与效率间取得良好平衡，适合资源受限边缘AI平台部署。
---

## 摘要
音视频事件识别（AVER）通过基于Transformer的多模态架构取得了显著的性能提升。然而，这些模型的高计算复杂度、大内存占用和推理成本阻碍了其在边缘设备和资源受限设备上的部署。本文提出了一种结合架构模型压缩、知识蒸馏和动态INT8量化的高效压缩框架，用于基于混合交叉注意力的音视频事件识别。高容量教师模型集成了用于视觉表示学习的VideoMAE、用于音频特征提取的音频频谱图Transformer（AST）以及用于多模态特征融合的混合交叉注意力融合网络。通过减小隐藏特征维度、注意力头数和前馈网络大小，同时保持整体网络架构，构建了轻量级学生模型。学生模型使用知识蒸馏进行训练，以有效从教师模型迁移判别性知识。最后，应用动态INT8训练后量化进一步减小模型尺寸，以实现高效部署。在音视频事件（AVE）数据集上的实验结果表明，所提框架将多模态融合模块的可训练参数减少了59.06%，与教师模型相比，分类准确率仅下降2.14%。此外，动态INT8量化将模型大小从10.71 MB减小到2.04 MB，同时保持了有竞争力的识别性能。这些结果表明，所提框架在识别准确率和计算效率之间提供了有效的权衡，使其成为在资源受限的边缘AI平台上部署的有前景的解决方案。
